            ...
            <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
                {
                    friends.map((item, i) => (
                        <div
                            key={i}
                            onClick={() => handleNavigateToProfile(item.id)} // Replace with your routing logic (e.g., navigate(`/user/${item.id}`))
                        ...
                        >
                            <div className="flex min-w-0 items-center gap-3.5">
                                <div className="relative">
                                    <div className={`flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br ${item.gradient || 'from-brand to-accent'} text-sm font-bold text-white shadow-inner border border-background/10`}>
                                        {item.initials}
                                    </div>
                                    <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-surface bg-green-500" />
                                </div>
                                <div className="min-w-0">
                                    <p className="truncate text-sm font-bold text-text-primary group-hover:text-brand transition-colors">{item.name}</p>
                                    <p className="truncate text-xs text-text-secondary mt-0.5">{item.meta}</p>
                                </div>
                            </div>
